...
public class FileDownloadController {
	...
	public ResponseEntity<byte[]> fileDownload_po(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			download = OperationFileUtil1.download(po.getPoPath());
			po.setOldOrNewPo("");
	...
	public ResponseEntity<byte[]> fileDownload_invoice(HttpServletRequest request, HttpServletResponse response){
		...
		ResponseEntity<byte[]> download = null;
		try {
			download = OperationFileUtil1.download(invoiceInfo.getInvoicePath());
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	try {
		...
		ResponseEntity<byte[]> download = null;
			download = OperationFileUtil1.download(qcReport.getQcReportPath());
	...
	public ResponseEntity<byte[]> fileDownload_shipping(HttpServletRequest request, HttpServletResponse response) {
	   ...
	   if(StringUtils.isBlank(request.getParameter("id"))){
		...
		if(StringUtils.isNotBlank(path)){
			download = OperationFileUtil1.download(path);
    ...
    public ResponseEntity<byte[]> fileDownload_clientDrawing(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException, IOException {
    	...
    	String drawingPath =  UploadAndDownloadPathUtil.getOldDrawingUpLoadAndDownLoadPath() + clientDrawings.getDrawingsName();
    	ResponseEntity<byte[]> download = OperationFileUtil1.download(drawingPath);
    	return download;
    }
...
//    	 files.add(file1);
    ...
    public ResponseEntity<byte[]> fileDownload_quotation(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			ResponseEntity<byte[]> download = OperationFileUtil1.download(quotationPath);			
			LOG.info("=========下载报价单成功=========");				 
    ...
    public ResponseEntity<byte[]> fileDownload_quotation_attachment(HttpServletRequest request, HttpServletResponse response) throws Exception {
    	...
    	if(!(StringUtils.isBlank(attachmentPath))){
        	download = OperationFileUtil1.download(attachmentPath);		
    	}					 
    ...
    public ResponseEntity<byte[]> fileDownload_document(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException, IOException{
		...
		ResponseEntity<byte[]> download = OperationFileUtil1.download(productionPhotoTimeline.getDocumentPath());
    	return download;
    }
}
